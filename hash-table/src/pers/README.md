...
> 散列函数
> 1. 直接定址法：关键码和地址之间存在某个线性函数。 H(key) = a * key + b。 a和b是常数，需要事先直到 key 是什么
> 2. 除留余数法：选取适当的正整数p， H(key) = key mod p。 
> 3. 平方取中法：421^2 为 177241， 去中间两位72做为哈希值 
> 4. 随机数法：H(key) = random(key)

> 哈希冲突的处理方式
> 1. 再哈希
> 2. 缓冲区：建立一个缓冲区，凡是哈希冲突的都放入缓冲区，如果通过key查找时，发现数据不对，则从缓冲区中查找
> 3. 开放定址法：从发生冲突的位置的下个位置开始找空的位置
> 4. 拉链法：用内容为链表的数组存储值
